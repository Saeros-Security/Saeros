...
namespace Collector.Integrations.Implementation.ElasticSearch;

internal sealed class ElasticSearchIntegrationService(ILogger logger, int integrationId, string integrationName, HashSet<DetectionSeverity> severities, ISystemAuditService systemAuditService, IIntegrationRepository integrationRepository, ElasticSearchSettings searchSettings)
    : IIntegration
{
    ...
    private static ElasticsearchClient CreateClientInternal(ElasticSearchSettings searchSettings)
    {
    ...
    public async Task SendAsync(IList<Export> exports, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                logger.Throttle(nameof(ElasticSearchIntegrationService), itself => itself.LogError(response.DebugInformation), expiration: TimeSpan.FromMinutes(1));
