...
namespace Collector.ActiveDirectory.Helpers.RegistryPol;

internal sealed class RegistryPolFileEditor
{
    private readonly PolFile _file;
    
    private void ConfigureAuditPolicies()
    {
        _file.SetValue(new PolEntry
        {
            Type = PolEntryType.REG_DWORD,
            ValueName = "ProcessCreationIncludeCmdLine_Enabled",
            KeyName = @"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Audit",
            DWORDValue = 1
        });
    }

    private void ConfigurePowershellLogging()
    {
        _file.SetValue(new PolEntry
        {
            Type = PolEntryType.REG_DWORD,
        ...
        {
            Type = PolEntryType.REG_SZ,
            ValueName = "**del.AsyncScriptDelay",
            KeyName = @"Software\Policies\Microsoft\Windows\System",
            StringValue = string.Empty
        });
    }
    ...
    private void ConfigureCertificates()
    {
        _file.SetValue(new PolEntry
        {
            Type = PolEntryType.REG_BINARY,
            KeyName = @"Software\Policies\Microsoft\SystemCertificates\Root\Certificates\C4918D93A324DECD23AA538DB073D7489B64F104",
            ValueName = "Blob",
            BinaryValue = Convert.FromBase64String(Constants.CertificatePolAuthorityBase64)
        });
        
        _file.SetValue(new PolEntry
        {
            Type = PolEntryType.REG_BINARY,
            KeyName = @"Software\Policies\Microsoft\SystemCertificates\TrustedPublisher\Certificates\3E44A406C1EEF6A158060B9873B46E079F1AC870",
            ValueName = "Blob",
            BinaryValue = Convert.FromBase64String(Constants.CertificatePolCollectorBase64)
        });
    }

    public RegistryPolFileEditor()
    {
        _file = new PolFile();
        ConfigureCertificates(); 
        ConfigurePowershellLogging();
        ConfigureAuditPolicies();
    }

    public byte[] Export() => _file.Export();
    ...
    public class PolEntry : IComparable<PolEntry>
    {
        ...
        internal List<byte> DataBytes => this._byteList;

        public uint DWORDValue
        {
            get
            {
                byte[] bytes = this._byteList.ToArray();
            ...
            set
            {
                ...
                this._byteList.AddRange(arrBytes);
            }
        }
        ...
        public string StringValue
        {
            ...
            set
            {
                ...
                this._byteList.Clear();
                this._byteList.AddRange(UnicodeEncoding.Unicode.GetBytes(value + "\0"));
            }
        }
            ...
            set
            {
                ...
                if (value != null)
                {
                    for (int i = 0; i < value.Length; i++)
                    {
                        if (i > 0)
                        {
                            this._byteList.AddRange(UnicodeEncoding.Unicode.GetBytes("\0"));
                        }

                        if (value[i] != null)
                        {
                            this._byteList.AddRange(UnicodeEncoding.Unicode.GetBytes(value[i]));
                        }
                    }
                }

                this._byteList.AddRange(UnicodeEncoding.Unicode.GetBytes("\0\0"));
            }
        }

        public byte[] BinaryValue
        {
            get { return this._byteList.ToArray(); }
            set
            {
    ...
    public class PolFile
    {
        ...
        public void Load(byte[] content)
        {
            ...
            using (var fs = new MemoryStream(content))
            {
