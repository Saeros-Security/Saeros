...
public static class RuleDeletionBroadcaster
{
    private static readonly ConcurrentHashSet<Channel<RuleIdContract>> Channels = new();
    private static readonly ConcurrentCircularBuffer<RuleIdContract> Contracts = new(capacity: 100);
    private static int _initialized;
    
    public static void RemoveDeletion(this Channel<RuleIdContract> channel) => Channels.TryRemove(channel);

    public static Channel<RuleIdContract> SubscribeDeletion(this IStreamingRuleHub hub, IHostApplicationLifetime applicationLifetime)
    {
    ...
    private static async Task BroadcastAsync(IStreamingRuleHub hub, CancellationToken cancellationToken)
    {
        try
        {
            await foreach (var message in hub.RuleDeletionChannel.ReadAllAsync(cancellationToken))
            {
