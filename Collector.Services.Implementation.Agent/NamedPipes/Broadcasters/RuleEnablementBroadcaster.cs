...
public static class RuleEnablementBroadcaster
{
    ...
    public static void RemoveEnablement(this Channel<RuleIdContract> channel) => Channels.TryRemove(channel);

    public static Channel<RuleIdContract> SubscribeEnablement(this IStreamingRuleHub hub, IHostApplicationLifetime applicationLifetime)
    {
    ...
    private static async Task BroadcastAsync(IStreamingRuleHub hub, CancellationToken cancellationToken)
    {
        try
        {
            await foreach (var message in hub.RuleEnablementChannel.ReadAllAsync(cancellationToken))
            {
